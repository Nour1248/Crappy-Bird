...
public:
  explicit pipe (str path, int distance_x = 0, int distance_y = 0)
  {
    ...
    ps.setTexture (p);
    ps.setPosition (
        sf::Vector2f ((WIDTH - 52) + distance_x, (HEIGHT - 320) + distance_y));
  }

  //////////////////////////////////////////////////////////////////////////////////////////

  static void
  create_bottom_pipes (pipe *arr[], str path)
  {
  ...
  static void
  create_upper_pipes (pipe *arr[], str fpath)
  {
    for (size_t i = 0; i < pipes_count; i++)
      {
  ...
  static void
  draw_pipes (pipe *arr[], sf::RenderWindow window)
  {
    for (size_t i = 0; i < pipes_count; i++)
      {
  ...
  static bool
  hover (sf::Sprite s, sf::Vector2f mouse_coordinates)
  {
